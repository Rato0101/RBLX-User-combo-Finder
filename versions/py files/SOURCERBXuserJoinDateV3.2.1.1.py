...
class MetroApp(tk.Tk):
    def __init__(self):
        ...
        self.title("Roblox Finder V3.2.1- by CloudKernelFox")
        ...
        self._error_results = []
        # secret click counter for Start Scan button
        self._start_scan_clicks = 0
    ...
    def _build_ui(self):
        ...
        self.header_label = tk.Label(
            header_frame,
            text="Roblox User & Combo Finder V3.2.1\nBy CloudKernelFox",
        ...
        # Save generated list (enabled after generation)
    ...
    def load_file(self):
        ...
        self.title(f"Roblox Finder V3.2.1 - {combo_count} combos loaded")

    def start_scan(self):
        # increment secret click counter and maybe show the secret popup
        try:
            self._start_scan_clicks += 1
        except Exception:
            self._start_scan_clicks = 1
        self._maybe_show_secret_popup()
    ...
    def _maybe_show_secret_popup(self):
        """If Start Scan was clicked 5 times, show a harmless secret popup."""
        if getattr(self, '_start_scan_clicks', 0) >= 5:
            # reset counter so it only triggers once per 5 clicks unless clicked again
            self._start_scan_clicks = 0
            ...
            lbl = tk.Label(popup, text=":)", font=("Comic Sans MS", 20), bg=BG_COLOR, fg=TEXT_COLOR)
            lbl.pack(pady=(18, 8))

            def on_recycle_click():
                # Harmless action: show an informational message and close the popup
                messagebox.showinfo(":)", "Recycling complete. :)")
                try:
                    popup.destroy()
                except Exception:
                    pass

            btn = ttk.Button(popup, text="recycle system 32", style="Metro.TButton", command=on_recycle_click)
            btn.pack(pady=(4, 12))

            # Ensure the popup is modal-ish
    ...
    def _lookup_public_users(self):
        ...
        for idx, line in enumerate(lines, 1):
                        ...
                        if code == 0:
                            ...
                            if users_resp.status_code == 200:
                                ...
                                if udata.get('data') and udata['data'][0]:
                                    ...
                                    result = f"[FOUND] {uname} (ID: {uid}) - joined: {join_str}\n"
                                    tag = 'found'
                                    # Store tuple with parsed join date (ISO), username, id, combo for later sorting
                                    ...
                                    # try to convert join_str back to ISO for sorting if possible
                                    try:
                                        from datetime import datetime
                                        # if join_str is formatted like dd/mm/YYYY from earlier parsing, try to invert
                                        if '/' in join_str:
                                            d, m, y = join_str.split('/')
                                            iso = f"{y}-{m.zfill(2)}-{d.zfill(2)}"
                                            self._found_results[-1]['joined_iso'] = iso
                                        else:
                                            # assume it's already ISO or parseable
                                            dt = datetime.fromisoformat(joined.replace('Z', '+00:00'))
                                            self._found_results[-1]['joined_iso'] = dt.date().isoformat()
                                    except Exception:
                                        # leave joined_iso as None if parsing fails
                                        pass
                                else:
            ...
            self.after(0, lambda left=combos_left: self.title(
                f"Roblox Finder V3.2.1 - {left} combos left" if left > 0 else "Roblox Finder V3.2.1 - Scan complete"
            ))

        self._thread_safe_btns_reset()
        if not self._scan_cancelled.is_set():
            self._thread_safe_messagebox("Done", "Scan completed.")
            self.after(0, lambda: self.title("Roblox Finder V3.2.1 - Scan complete"))
            self.after(0, lambda: self.combos_label.config(text="Combos: 0"))
            # enable Save Generated (Save All button removed)
            self.after(0, lambda: self.save_generated_btn.config(state="normal"))
    ...
    def save_results(self):
        ...
        try:
            ...
            content = ""
        if not content and not getattr(self, '_found_results', None):
            messagebox.showinfo("No Results", "No results to save.")
            return

        # If we have found results with combos, write a sorted FOUND section first
        ...
        try:
            with open(file_path, "w", encoding="utf-8") as f:
                # Write FOUND entries sorted by join date (oldest -> newest) if available
                found = getattr(self, '_found_results', []) or []
                parsed = []
                from datetime import datetime
                for item in found:
                    # item may be a dict (new) or a preformatted string (old). Handle both.
                    if isinstance(item, dict):
                        iso = item.get('joined_iso')
                        joined_raw = item.get('joined_raw') or ''
                        ...
                        if iso:
                            try:
                                ...
                                try:
                                    ...
                                    dt = None
                        parsed.append((dt, username, uid, joined_raw, combo))
                    else:
                        # attempt to parse legacy save_line string
                        # format: "[FOUND] {uname}  -ID.{uid}   -joined.{join_str}  {combo}"
                        try:
                            s = item
                            # crude parsing
                            parts = s.split('-ID.')
                            uname_part = parts[0].replace('[FOUND]', '').strip()
                            rest = parts[1].split('  -joined.')
                            uid = rest[0].strip()
                            joined_part = rest[1].strip()
                            if '  ' in joined_part:
                                joined_str, combo = joined_part.split('  ', 1)
                            else:
                                joined_str = joined_part
                                combo = ''
                            dt = None
                            try:
                                # try dd/mm/YYYY
                                d, m, y = joined_str.split('/')
                                dt = datetime(int(y), int(m), int(d))
                            except Exception:
                                try:
                                    dt = datetime.fromisoformat(joined_part.replace('Z', '+00:00'))
                                except Exception:
                                    dt = None
                            parsed.append((dt, uname_part, uid, joined_str, combo))
                        except Exception:
                            # fall back to writing raw content at end
                            parsed.append((None, item, '', '', ''))

                # sort with None dates at the end
                parsed.sort(key=lambda x: (x[0] is None, x[0] or datetime.max))

                if parsed:
                    f.write("== FOUND (sorted by join date oldest->newest) ==\n")
                    for dt, uname, uid, jraw, combo in parsed:
                        if combo:
                            f.write(f"{uname}\tID:{uid}\tjoined:{jraw}\tcombo:{combo}\n")
                        else:
                            f.write(f"{uname}\tID:{uid}\tjoined:{jraw}\n")
                    f.write("\n")

                # then write the rest of the results as raw text (if any)
                if content:
                    f.write("== RAW OUTPUT ==\n")
                    f.write(content + "\n")
    ...
    def save_all(self):
        ...
        if not file_path:
            return
        try:
            # Build FOUND parsed list similar to save_results
            with open(file_path, 'w', encoding='utf-8') as f:
                found = getattr(self, '_found_results', []) or []
                parsed = []
                from datetime import datetime
    ...
    # Sorting-by-button functionality removed per user request (Sort Generated, Sort File, Save Sorted)
